...
def create_event(request):
    ...
    if request.method == 'POST':
        ...
        if response_new_event_form.is_valid():
            ...
            Event.objects.create( event_name = event_name, timestamp = date_event,duration = duration, coordinator = coordinator, description = description)
            print("saved event")
...
def edit_event(request, event_id):
    ...
    if request.method == 'POST':
    	changed_event_details = EventForm(request.POST, initial= event_data)
    	if changed_event_details.has_changed() and changed_event_details.is_valid():
    		...
    		Event.objects.filter(pk=event_id).update( event_name = event_name, timestamp = date_event,duration = duration, coordinator = coordinator, description = description)
    		print("saved event")
    		return HttpResponseRedirect(reverse('coord_main_panel'))
