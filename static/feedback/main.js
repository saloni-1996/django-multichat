$(function() {
  ...
  socket.onmessage = function(message) {
    ...
    if (data.join) {
      ...
      var roomdiv = $("<div class='room' id='room-" + data.join + "'>" + "<h2>" + data.title + "</h2>" + "<div class='messages'></div>" + "<input id='message'><button id='send'>Send</button>" + "</div>");
      $("#chats").append(roomdiv);
      roomdiv.find("input").keypress(function(e) {
        var code = (
          e.keyCode
          ? e.keyCode
          : e.which);
        if (code == 13) {
          roomdiv.find("button").trigger('click');
          return true;
        }
      });

      roomdiv.find("button").on("click", function() {
        socket.send(JSON.stringify({"command": "send", "room": data.join, "message": roomdiv.find("input").val()}));
